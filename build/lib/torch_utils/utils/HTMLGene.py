...
<body>
    ...
    <tr align="center">
        ...
        <td> <img src={{ image.label_path }} height="128"> </td>
        <td> <img src={{ image.prediction_path }} height="128"> </td>
...
class ModelParas:
    ...
    def add_image(self, image: ImageParas):
        ...
        if not os.path.exists("{}/images/label".format(DIR_PATH)):
            os.makedirs("{}/images/label".format(DIR_PATH))
        if not os.path.exists("{}/images/prediction/{}".format(DIR_PATH, self.model_name_ul)):
            os.makedirs("{}/images/prediction/{}".format(DIR_PATH, self.model_name_ul))

        raw_path = "{}/images/raw/{:05}.png".format(DIR_PATH, image.img_id)
        label_path = "{}/images/label/{:05}.png".format(DIR_PATH, image.img_id)
        prediction_path = "{}/images/prediction/{}/{:05}.png".format(DIR_PATH, self.model_name_ul, image.img_id)

        if not os.path.exists(raw_path):
            cv2.imwrite(raw_path, image.raw)
        if not os.path.exists(label_path):
            cv2.imwrite(label_path, image.label)
        if not os.path.exists(prediction_path):
            cv2.imwrite(prediction_path, image.prediction)

        image.raw_path = "images/raw/{:05}.png".format(image.img_id)
        image.label_path = "images/label/{:05}.png".format(image.img_id)
        image.prediction_path = "images/prediction/{}/{:05}.png".format(self.model_name_ul, image.img_id)
...
def example():
    ...
    raw = torch.zeros((3, 100, 100)).type(torch.FloatTensor)
    label = torch.zeros((100, 100)).type(torch.FloatTensor)
    prediction = torch.zeros((100, 100)).type(torch.FloatTensor)
    ...
    image1 = ImageParas(
        ...
        metrics_value=[0.2, 0.3]
    )
    image2 = ImageParas(1, raw, label, prediction, [0.5, 0.6])
    ...
    model1.add_image(image2)
    model1.add_image(image1)

    model2 = ModelParas("Second model", "paper url2", "paper name2", "code url2")
    model2.add_image(image1)
    model2.add_image(image2)

    paras.add_model(model1)
    paras.add_model(model2)
